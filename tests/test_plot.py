...
def test_plot_with_n_factors_to_plot_subset(factor_matrix):
    ...
    fig = plot_nmf_factor_distributions(factor_matrix, n_factors_to_plot=3, return_fig=True)
...
def test_plot_log_scales(factor_matrix):
    ...
    fig = plot_nmf_factor_distributions(factor_matrix, log_x=True, log_y=True, return_fig=True)
    assert fig is not None
...
def test_plot_filter_zeros(factor_matrix):
    ...
    W[:50, 0] = 0.0
    fig = plot_nmf_factor_distributions(W, filter_zeros=True, zero_threshold=1e-6, return_fig=True)
    assert fig is not None
...
def test_plot_with_factor_names(factor_matrix):
    ...
    fig = plot_nmf_factor_distributions(factor_matrix, factor_names=names, return_fig=True)
...
def test_plot_overall_title(factor_matrix):
    ...
    fig = plot_nmf_factor_distributions(factor_matrix, title="Custom Suptitle 123", return_fig=True)
...
def test_plot_subsamples_for_large_n(factor_matrix):
    ...
    fig = plot_nmf_factor_distributions(factor_matrix, max_samples=50, return_fig=True)
    assert fig is not None
...
def test_plot_custom_figsize_and_max_cols(factor_matrix):
    ...
    fig = plot_nmf_factor_distributions(factor_matrix, figsize=(10, 6), max_cols=2, return_fig=True)
    assert fig is not None
...
def test_plot_shared_axes(factor_matrix):
    ...
    fig = plot_nmf_factor_distributions(factor_matrix, sharex=True, sharey=True, return_fig=True)
    assert fig is not None
...
def test_plot_handles_all_zero_factor():
    ...
    W[:, 0] = 0.0
    fig = plot_nmf_factor_distributions(W, filter_zeros=True, zero_threshold=1e-3, return_fig=True)
    assert fig is not None
