...
def _make_batched_sparse(n_cells=120, n_genes=200, n_batches=3, k_true=5, seed=0):
    ...
    batch = np.repeat(np.array([f"b{i}" for i in range(n_batches)]), n_cells // n_batches)
...
def test_alignment_weight_extremes_shrink_v():
    ...
    assert strong_v_l2 < weak_v_l2, f"strong α_v should shrink V more: {strong_v_l2=}, {weak_v_l2=}"
